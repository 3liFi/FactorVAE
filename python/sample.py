import torch
from model.training import VAELightning
import matplotlib.pyplot as plt
import torchvision
from model.vae import HyperParams, LATENT_DIM
...
def load_trainer_from_model_path(model_path="../vae_model_factor_chest.ckpt", hyper_params=HyperParams()) -> VAELightning:
    ...
    Returns:
        VAELightning: The VAELightning instance
    """
...
def replicate_images(dataset, trainer: VAELightning, show=True, save=True) -> plt.Figure:
    """
    ...
    Args:
        ...
        trainer: The VAELightning instance containing the model that should be used to replicate the images
        show (Bool): Whether the generated plot should be shown in a pop-up window
        save (Bool): Whether the generated plot should be saved to a file

    Returns:
        matplotlib.figure.Figure: The figure containing the generated images
    """
...
def sample_latent_changes(dataset, trainer: VAELightning, show=True, save=True) -> plt.Figure:
    """
    ...
    # Add difference maps
    return visualize_with_difference_maps(reconstructed, group_size=len(latent_vals), show=show, save=save)


def visualize_with_difference_maps(images, group_size=13, threshold=0.02, show=True, save=True) -> plt.Figure:
    """
    ...
    # Show plot
    fig = plt.figure(figsize=(images_per_row, LATENT_DIM))
